...
Live loop now runs `paper_trade.py --rl --champions` (online Q-learning + champion routing). It writes status to:
...
## RL Self-Train (Champion + Challenger)

Runs historical walk-forward RL with automatic promotion gates:
- Global champion
- Regime champions (`bull`, `bear`, `range`)
- Two candidate programs:
- `explore` (idea discovery)
- `champion_refine` / `refine` (improve proven ideas)

```
run_rl_self_train.bat
```

Outputs:
- `data/models/rl_self_train_status.txt`
- `data/models/rl_champions.csv`
- `data/models/rl_champions_summary.txt`
- `data/models/champions/<regime>/...`
- `data/models/rl_strategy_pool.csv`
- `data/models/rl_strategy_recheck.csv`
- versioned experiment runs under `data/models/experiments/<run_id>/`
- experiment index at `data/models/experiments/index.csv`

Notes:
- Uses purged walk-forward splits (`purge_bars`) to reduce train/test boundary leakage.
- Promotions are score/improvement-based with minimum window coverage (no fixed drawdown/hit-rate thresholds).

## Versioned experiment tracking

Every RL training run now writes a versioned snapshot:
- `metadata.json` (run id, source, parent, artifacts)
- `params.json` (hyperparameters and split settings)
- `metrics.json` (objective and train/test metrics)
- copied artifact files used/generated by the run

Useful files:
- latest pointer: `data/models/rl_last_run.txt`
- run index: `data/models/experiments/index.csv`

Optional tags for custom runs:
```
.\.python\python.exe scripts\rl_train_historical.py --exp-tag baseline_v1 --exp-note "return-only objective"
```

## RL Rationale Assessment

Each RL training run now audits its own rationale patterns (state context + action),
checks whether outcomes matched, and stores that memory for later cycles.

Outputs:
- `data/models/rl_rationale_assessment.csv`
- `data/models/rl_rationale_summary.txt`
- `data/models/rl_rationale_memory.csv`
...
RL objective defaults to return-only. You can optionally add drawdown penalty:
```
.\.python\python.exe scripts\rl_train_historical.py --dd-penalty 0.5
```
